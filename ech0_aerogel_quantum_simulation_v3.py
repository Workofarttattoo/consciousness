#!/usr/bin/env python3
"""Deterministic estimator for the ECH0 V3 (CNT-reinforced) aerogel recipe."""
...
def main() -> dict:
    recipe = AerogelRecipe(
        name="ECH0 V3 CNT-reinforced",
        freeze_temp_c=-80.0,
        base_material="sodium_silicate",
        pva_mass_fraction=0.25,
        mtms_wt_percent=7.5,
        glutaraldehyde_vol_percent=1.2,
        additives={"cnt": 0.3},
    ...
    report = {
        ...
        "model": "deterministic_v3_estimator",
        "recipe": {
            "name": recipe.name,
    ...
    output_path = Path("~/repos/consciousness/ech0_aerogel_simulation_results_v3.json").expanduser()
