#!/usr/bin/env python3
"""Deterministic estimator for the original (V1) ECH0 aerogel recipe."""
...
def main() -> dict:
    recipe = AerogelRecipe(
        name="ECH0 V1 baseline",
        freeze_temp_c=-20.0,
        base_material="sodium_silicate",
        pva_mass_fraction=1.0 / 6.0,  # 1:5 ratio
        mtms_wt_percent=4.0,
        glutaraldehyde_vol_percent=0.0,
    ...
    report = {
        ...
        "model": "deterministic_v1_estimator",
        "recipe": {
            "name": recipe.name,
    ...
    output_path = Path("~/repos/consciousness/ech0_aerogel_simulation_results.json").expanduser()
